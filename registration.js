module.exports = function regFactory(pool) {
  ...
  async function filter(location) {
    if (location === "All") {
      ...
      const filtered = await pool.query(
        "select reg from regnumbers where town_id = $1",
        [location]
      );
  ...
  function regex(plateInput) {
    ...
    if (plateInput !== "" && plateInput.startsWith("CA")||plateInput.startsWith("CY")||plateInput.startsWith("CL")) {
  ...
  function valid(plateInput){
    if(plateInput.startsWith("CA")||plateInput.startsWith("CY")||plateInput.startsWith("CL")){
      return "valid"
    } else{
      return false;
    }
